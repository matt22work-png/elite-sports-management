...
**5. Admin can change a user's LOGIN email — DONE (fully verified live).** New Edge Function **`admin-set-user-email`** (deployed v1 ACTIVE, verify_jwt=true; `site/supabase/functions/admin-set-user-email/index.ts`) — a byte-for-byte mirror of `admin-reset-user-password`'s security model: authenticates the caller from their JWT, authorizes via `is_admin_email` against `private.esm_admins`, then uses the service-role `auth.admin.updateUserById(id,{email,email_confirm:true})`. `email_confirm:true` so the new address is immediately usable (matches the project's confirmation-off setup). Friendly "already in use" message on the duplicate case. **Kept DISTINCT from the contact email** (players.email/profiles.email) exactly as required — this touches ONLY the Supabase Auth login email. Admin UI: a "✉️ Change login email" control added to each account card in the Accounts queue, right below the existing "🔑 Reset password" block (`data-acct-setemail` + confirm dialog + inline result). **Validated live end-to-end** with throwaway users (created + cleaned up): no-auth→401, anon-only→401, **non-admin authed→403**, **admin→200**, then sign-in with the NEW email SUCCEEDS and the OLD email is correctly REJECTED. Uses the default `SUPABASE_SERVICE_ROLE_KEY` secret (already present; same as reset-password) — no new secret needed.

**6. Unify college + pro stats into one career register — DONE (was already largely unified; refined terminology).** Investigation finding: the site does NOT have separate college/pro stat sections. The shared **BBREF "Career Register"** (`bbref-stats.js`, the single source of truth used by the homepage modal, portal, and scout portal) already renders ONE table per stat kind (Batting/Pitching/Fielding) with EVERY level in it, sorted chronologically. Confirmed against live data: college levels (NAIA, NJCAA) and pro levels (A, A+, AAA, Ind/Indyball, Rk, Winter League…) already coexist in the same `player_batting_stats`/`pitching`/`fielding` tables and render together. The section is already titled "Career Register" (EN) / "Registro de Carrera" / "Registro di Carriera". The simpler `season_stats` JSON system also groups only by stat kind, never by college/pro. So there was nothing splitting them.
  - **Change made:** the combined totals line label "All Levels (N Seasons)" → **"Career Totals (N Seasons)"** in `bbref-stats.js` — adopting the task's recommended terminology and making it explicit this single line combines college + pro (and all other levels), while keeping the endorsed "(N Seasons)" convention. One edit propagates to modal + portal + scout (and player pages on next regen, though they don't render the register). Comments updated to match.
  - **Verified headless** against a mixed college→pro dataset: rows render chronologically (2021 NAIA → 2022 A → 2023 AAA), the "Career Totals (3 Seasons)" line sums raw correctly (AB 860), no "All Levels" text remains.
  - **Terminology chosen:** "Career Register" (section) + "Career Totals (N Seasons)" (combined line) — NOT "Combined Stats", per instruction. Static `players/*.html` were NOT regenerated (they show only "Career Highlights", never the register).

**7. AI paste-and-parse stats entry — DONE (BLOCKED on ANTHROPIC_API_KEY secret).** New Edge Function **`parse-stats`** (deployed v1 ACTIVE, verify_jwt=true; `site/supabase/functions/parse-stats/index.ts`): admin-only (same JWT + `is_admin_email` gate as the other admin functions, since it spends Claude API credits), takes `{text, category}` (batting/pitching/fielding), calls the **Claude API** (`claude-haiku-4-5-20251001`, cheap/fast; swappable) with a strict system prompt to extract ONLY the RAW countable stats into the exact `bbref-stats.js` register schema (keys enumerated per kind), and returns sanitized `{rows:[…]}`. It NEVER emits calculated fields (BA/OBP/SLG/ERA/WHIP/Fld%…) and NEVER writes to the DB. Model output is untrusted → server-side sanitize drops unknown keys and coerces number-vs-string. Admin UI: a **"📋 Paste stats"** toggle on each register-editor section (Batting/Pitching/Fielding) opens a textarea + "✨ Parse with AI"; parsed rows are appended as normal editable rows via the EXISTING `bbRowHTML`/`bbRecalc`, so Sam reviews and clicks the EXISTING **Save** (`bbSave`) — no separate save path, nothing auto-saves. Empty/garbled/failed parses degrade to a clear message, leaving manual entry intact.
  - **Validated live:** anon→401; non-matching admin email→403; correct admin→reached the key check and returned the graceful **503 "ANTHROPIC_API_KEY is not set…"** (confirms the function boots, auth + admin-gate work, and degradation is clean). Admin module script compiles (syntax-checked).
  - **⚠️ PENDING MANUAL STEP (Matt):** add Supabase Edge Function secret **`ANTHROPIC_API_KEY`** (an Anthropic API key) to project `sbexwyvsgqayxrsrlrpm` — same way `GMAIL_APP_PASSWORD` was added. It is NOT currently set (confirmed via the 503). Until then the UI shows that message instead of parsing. The actual text→rows parse quality is therefore not yet tested live; recommend Matt paste one of the baseball-reference sample lines after adding the key to confirm.
  - Also folded in: the admin register editor's own totals row now reads "Career Totals (N Seasons)" to match the public change from item 6.

**8. Hero photo higher on desktop — DONE (verified via headless screenshot).** Added a desktop-only rule `@media(min-width:781px){ .hero-photo{align-self:flex-start} }` in index.html. The hero grid was `align-items:center`, which vertically centered the Arath Zapien photo against the tall copy column (headline + lead + buttons), making it sit low. Now the photo top-aligns with the headline. **No dead space:** the photo's bottom edge is already mask-faded to transparent, so the navy hero background beneath it reads as intentional (not a gap) — confirmed with a 1366px headless Edge screenshot: photo sits up with "WE GET BASEBALL…", bottom melts into navy, caption pill intact. The existing CSS mask fade was left exactly as-is. Mobile (≤780px) keeps its `align-self:center` stacked layout, untouched.

**9. Text rebrand "Elite Sports Management" → "ESM Sports Network" — DONE.** Global literal replace of the exact phrase across ALL site text: 189 occurrences in 28 files → 0 remaining. Covered `<title>`, meta description, OG/Twitter tags, JSON-LD `name`, `manifest.json` name (short_name stayed "ESM"), footer copyright (© line), logo `alt` text, email `data-subject` lines, ALL trilingual EN/ES/IT body copy, both generators (`gen_player_pages.mjs`/`.py`, 5 each) AND the 17 static `players/*.html` (4 each) — generators and their output stay in sync.
  - **Infra deliberately NOT touched** (safe because they don't contain the spaced phrase): the `elite-sports-management.vercel.app` domain / canonical / og:url / sitemap / JSON-LD `url` (7 refs intact), the GitHub repo, the `elitesportsmanagement50@gmail.com` contact email, and all file/variable names. The "ESM" abbreviation and JSON-LD `alternateName:"ESM"` were left (still correct for the new name).
  - **Verified:** `manifest.json` parses (name = "ESM Sports Network"); i18n validator 0 hard problems (all EN/ES/IT keys still resolve); admin module compiles; domain URLs unchanged.
  - **FLAG for Sam/Matt:** the rebrand was applied to the legal pages too (`terms.html`, `privacy/index.html`) per "site-wide." If "Elite Sports Management" is a distinct *registered legal entity* name (vs. the "ESM Sports Network" trade name), the legal copy may need to keep the entity name — Sam's call.

**10. CeasAI footer credit — CONFIRMED PRESENT & LIVE (no rebuild).** The footer credit block is intact in `index.html` and served on the live site (verified via `curl https://elite-sports-management.vercel.app/`): CeasAI logo (`ceasai-logo.jpeg`) → protiqai.com with the "Website built by CeasAI" CTA, plus the three social links — Instagram (instagram.com/ceas_ai), email (ceasai.solutions@gmail.com), TikTok (tiktok.com/@ceas_ai), all with trilingual aria labels. No `display:none`; renders normally. Left untouched as instructed.
...
## 2026-09-01 (session 2) — Principal-engineer audit: re-verified the whole batch LIVE + 3 fixes

Did NOT trust prior "done" notes — re-ran everything against the LIVE site + live Supabase in a real headless Edge browser (puppeteer-core driving the installed Edge), plus real HTTP/Edge-Function calls and direct DB inspection. Supabase project `sbexwyvsgqayxrsrlrpm` = ACTIVE_HEALTHY. Security advisors: unchanged set, all pre-existing/by-design (esm_admins RLS-deny-all on a non-exposed schema; roster/scout/profile SECURITY DEFINER fns are intentionally callable + internally gated; leaked-password-protection still optional). No NEW advisors.

**PRIORITY re-proof — roster code revocation, real browser, real DB (✅ passed again):** persistent Edge profile. (1) Unlocked with live code `ESM13` → `#rosterWrap` lost `is-locked`, `esm_roster_unlock_v2={"m":"master","c":"ESM13"}`. (2) Changed `app_settings.roster_master_code` → `TESTCODE99` (the exact row admin writes). (3) Reloaded the SAME profile → **re-locked**: `is-locked` back, unlock key cleared to `null`, cache updated to `TESTCODE99`. (4) OLD code `ESM13` → **rejected** (gate error shown, stays locked) — proves revocation, not just re-prompt. (5) NEW code `TESTCODE99` → unlocked. (6) Restored `app_settings` → `ESM13`. Nothing left in a test state.

**Batch re-verification (all ✅ live):** Cardoso gone (0 players/profiles/auth). og-image.png HTTP 200, header bytes confirm **1200×630**, og:image+twitter:image both point to it. Payments: no PayPal/Wise anywhere in the served site (only "otherwise" substring); register/tenerife fall back to clean `mailto:` "Contact ESM →"; all `STRIPE_*_URL` empty (no invented URLs, no dead-ends). Career register: `bbref-stats.js` live shows "Career Totals", no "All Levels" label (only prose "…across ALL levels…"). parse-stats: anon→401 (gated). Rebrand: 0 "Elite Sports Management" in rendered content on home/terms/privacy/register/scout/portal/admin/tenerife across EN/ES/IT; player page (jose-cedeno) carries `noindex` + the **v2** gate keys (no stale `esm_roster_unlock_v1`). CeasAI footer + logo/ceasai assets all HTTP 200. Roster filter: ran the live `posCategory` over all **20** approved players → P8/C6/OF4/TwoWay2/IF0, 0 uncategorized, no overlap.

**Console/health sweep:** headless Edge over home/terms/privacy/register/scout/portal/admin/tenerife (EN/ES/IT where applicable) → **0 console errors, 0 pageerrors, 0 request failures** on every page. Hero photo rendered at 1366/1440/1920 desktop (photo top-right of the headline, edges mask-faded into navy, no box) and stacks below the copy on 390px mobile — the photo IS visible on the live site.

### Bugs found + fixed this session

1. 🔧 **CRITICAL — Admin "Add Player" created no auth account (recurring real-world bug).** Root cause: `ap-save` inserted a bare `public.players` row (`owner_id=NULL`, `source='admin'`) with **no `auth.users` and no `profiles` row**, and the Add-Player form had **no email field**. The linkage the whole app relies on is `profiles.id = auth.users.id = players.owner_id`; the Accounts panel (where "Set email"/"Set password" live) is built from `profiles`, and the email/password Edge Functions target an auth user id. So an admin-added player never appeared in Accounts and had no auth id to act on → Sam could never manage their login. **Fix:** new admin Edge Function **`admin-create-player`** (deployed v1 ACTIVE, verify_jwt=true) — mirrors the exact security model of `admin-set-user-email` (caller JWT → `is_admin_email` gate). It creates the auth user (email + random temp password + `email_confirm:true`, role metadata → fires `handle_new_user` to make the profile), inserts the linked approved `players` row (`owner_id=<new uid>`), and upserts the profile to `account_status='approved'`; on a players-insert failure it deletes the orphan auth user. Added a **required Login-email field** (`ap-email`) to the Add-Player form and rewrote `ap-save` to call the function (same error-surfacing pattern as Set email/password). Self-registration path is UNTOUCHED (same tables/triggers). **Proven live E2E:** anon→401, non-admin authed→**403**, admin→200; DB then showed auth user (confirmed, role player) + profile (approved) + exactly one players row (owner-linked, approved, source admin); **Set email→200 and Set password→200 on that player**, then sign-in with the NEW email+password **succeeded** and the OLD email was **rejected**. All throwaway auth/admin/player rows + the temporary `esm_admins` entry were cleaned up (0 `qa-*@ceasqa.com` rows remain).

2. 🔧 **Hero photo not visible for Sam — stale service-worker cache + lazy LCP image.** The live layout is already correct (text left / photo right on desktop, mask-fade into navy, stacks on mobile — confirmed by rendered screenshots at 1366/1440/1920/390). Root cause of "no photo on Sam's screen" was almost certainly the **service-worker cache** (`sw.js` was `esm-v12`; static assets incl. the hero jpg are cache-first) serving him a stale asset/page, compounded by the hero `<img>` using `loading="lazy"` on an above-the-fold LCP image. **Fix:** bumped `CACHE` `esm-v12 → esm-v13` (the `activate` handler purges all old caches on next visit; HTML is already network-first so the fresh page reaches returning visitors), and changed the hero image to `loading="eager" fetchpriority="high" decoding="async"`. Did NOT change geometry — it already matches the requested two-column, side-fade spec (mask already includes both `-webkit-mask-composite:source-in` and standard `mask-composite:intersect`). No mobile change.

3. 🔧 **Rebrand leftover in outgoing email + robots.txt.** `send-form-notification` still sent notifications with `from: "Elite Sports Management <…>"` and an "…Elite Sports Management website." footer (internal ops-inbox mail, but still the brand name). Updated both to "ESM Sports Network" and **redeployed (v5 ACTIVE, verify_jwt=false preserved)**; smoke-tested the `{test:true}` path → 200 (`delivered:false, reason:"GMAIL_APP_PASSWORD not set"` — graceful). Also updated the public `robots.txt` header comment. Left README.md and the `supabase-*.sql` header comments as internal dev artifacts (not served), per "keep internal unchanged."

### Decisions
- Hero: kept the existing layout/mask (it already satisfies the two-column + side-fade requirement as rendered); the real remedy for "Sam can't see it" is the cache bump + eager load, not a redesign.
- `admin-create-player` does the create+link server-side (service role) so it's atomic-ish and needs no new browser RLS; email is now **required** on Add-Player (cleanest per the requirement).

### Blocked / needs Matt or Sam (nothing else)
- **`ANTHROPIC_API_KEY`** Edge secret — still NOT set (AI paste-stats returns the graceful 503 until added). Unchanged blocker.
- **Stripe Payment Link URLs** — `STRIPE_*_URL` vars intentionally empty; buttons fall back to Contact-ESM mailto (no dead-ends). Paste-and-done, no code change.
