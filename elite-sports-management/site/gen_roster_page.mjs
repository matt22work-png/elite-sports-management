...
<!-- The roster is gated, so keep it out of search results exactly like the player pages. -->
...
${up(CSS)}
  ...
  /* .wrap supplies the 22px side padding; repeating it here is deliberate — this rule
     comes later in the cascade and a bare padding:34px 0 70px would wipe it out, leaving
     the page flush against the screen edge on anything narrower than 1124px. */
  .rp-main{padding:34px 22px 70px;position:relative;z-index:1}
  .rp-head{margin-bottom:30px}
  /* The homepage roster is a fixed 3-up grid inside a narrow column. This is a dedicated
     page, so it becomes a fluid wrapping grid with room to breathe. */
  #rosterList.roster{grid-template-columns:repeat(auto-fill,minmax(232px,1fr))}
  @media(max-width:560px){#rosterList.roster{grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:12px}}
  .rp-count{color:var(--muted);font-size:12.5px;text-align:center;margin:-4px 0 18px;letter-spacing:.04em}
</style>
</head>
<body>
...
<header class="nav"><div class="wrap nav-in">
  ...
  <div class="nav-right">
    ...
    <div class="lang" id="lang"><button type="button" data-l="en" class="on" aria-label="English">EN</button><button type="button" data-l="es" aria-label="Espa&ntilde;ol">ES</button><button type="button" data-l="it" aria-label="Italiano">IT</button></div>
...
<main class="wrap rp-main">
  ...
  <!-- Same client-side unlock gate as before, just relocated. An unlocked visitor sees
       ...
       /roster/ directly (or after a code rotation) is never a dead end. -->
...
<footer><div class="wrap foot">
  <div>
    ...
    <p class="muted">&copy; <span id="yr"></span> ESM Sports Network. <span data-i18n="foot_rights"></span></p>
  </div>
  <div class="foot-links">
    <a class="pill" href="https://www.instagram.com/esm_sports_network__01/" target="_blank" rel="noopener" aria-label="ESM Sports Network on Instagram">&#9678; @esm_sports_network__01</a>
    <a class="pill" data-mail>&#9993; <span id="footMail"></span></a>
  </div>
</div></footer>
...
document.getElementById("footMail").textContent=CONTACT_EMAIL;
